...
<script>
  export default {
        ...
        .then((resp) => {
          ...
          if (this.isLoggedId) {
            this.$store.dispatch('getEntity', {
              entity: 'rankings',
              data: { where: `product_id = ${this.product.id} AND user_id = ${this.isLoggedId}` }
            })
              .then((resp) => {
    ...
    methods: {
      runToPage(page) {
        this.$router.push(`/products/${this.product.id}?page=` + page).catch(()=>{});;
        this.initializeComment();
      },
      initializeComment () {
        ...
        let newData = {count: 'true', where: `entity_id = ${this.product.id} AND entity_type="product"`}

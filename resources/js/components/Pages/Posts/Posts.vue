<template>
  <v-container grid-list-md text-xs-center class="mt-5 mb-5">
    <v-layout row wrap>
      <v-flex xs12 mt-5>
        <h2 class="display-1 title-admin text-center text--secondary">Статьи</h2>
      </v-flex>
  ...
  export default {
    ...
    methods: {
      initialize() {  //
        ...
        this.$store.dispatch('getEntity', {entity: 'posts', data: {where: 'top_id is null', count: 'true'}})
          .then((resp) => {
            this.total= Math.ceil(resp.data / 10);
          })
          .catch(err => this.$router.push('/'));
        this.$store.dispatch('getEntity', {entity: 'posts', data: {where: 'top_id is null', offset, limit: 10, order: 'id desc'}})
          .then((resp) => {
      ...
      runToPage(page) {
        this.$router.push('/pages?page=' + page);
        this.initialize();
      }
    }
  }
